...
print("=" * 55)
print("CLIENT UDP - Retele de calculatoare")
print("=" * 55)
...
while True:
    ...
    if comanda == "EXIT":
        ...
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE LOCALA: Trebuie sa furnizezi un text dupa comanda PUBLISH.")
            continue
        ...
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE LOCALA: Trebuie sa furnizezi un ID dupa comanda DELETE.")
            continue

        if not parti[1].strip().isdigit():
            print("EROARE LOCALA: ID-ul trebuie sa fie un numar intreg.")
            continue
